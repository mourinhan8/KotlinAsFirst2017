...
fun sibilants(inputName: String, outputName: String) {
    ...
    for (i in 0..input.size - 1) {
        if (input[i] in list)
            for (j in 0..list1.size - 1) {
                if (input[i + 1] == list1[j].first) input[i + 1] = list1[j].second
            }
    }
    for (i in input) result.append(i)
    result.close()
}
...
fun transliterate(inputName: String, dictionary: Map<Char, String>, outputName: String) {
    val file = File(inputName).readText().toMutableList()
    ...
    val str = list1.joinToString("").toMutableList()
    str[0] = str[0].toUpperCase()
    val str2 = str.joinToString("")
    val file2 = File(outputName).bufferedWriter()
    file2.write(str2)
    file2.close()
}
